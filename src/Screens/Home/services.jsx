import React from "react";

const Services = () => {
    return (
        <div>

            <section className="index3-event-info-zone w-100 float-left padding-bottom padding-top">
                <div className="container">
                    <div className="generic-title2 text-center">
                        <span className="small-text">WHY MEETINGS21?</span>
                        <h2>Our Services and Highlights</h2>
                    </div>
                    <div className="index3-event-info-inner-con">
                        <div className="index3-event-zone">
                            <figure>
                                <img src={process.env.PUBLIC_URL + '/' + "images/event-zone-img1.png"} alt="event-zone-img1" />
                            </figure>
                            <h3>21+ Hours of Content</h3>
                            <p>Listen to the 21 hours of quality presentations by the experts in the field.</p>
                        </div>
                        <div className="index3-event-zone">
                            <figure>
                                <img src={process.env.PUBLIC_URL + '/' + "images/event-zone-img2.png"} alt="event-zone-img2" />
                            </figure>
                            <h3>World Class Speakers:</h3>
                            <p>Our conference features Top speakers as Plenary and Keynote Speakers.</p>
                        </div>
                        <div className="index3-event-zone">
                            <figure>
                                <img src={process.env.PUBLIC_URL + '/' + "images/event-zone-img3.png"} alt="event-zone-img3" />
                            </figure>
                            <h3>Live Q&A:</h3>
                            <p>Get answers and Clarify your doubts directly from the experts.</p>
                        </div>
                    </div>
                    <div className="index3-event-info-inner-con mt-5">
                        <div className="index3-event-zone">
                            <figure>
                                <img src={process.env.PUBLIC_URL + '/' + "images/awards.png"} alt="event-zone-img1" />
                            </figure>
                            <h3>Conference & Awards:</h3>
                            <p>We recognize and award the top presentations in the conference.</p>
                        </div>
                        <div className="index3-event-zone">
                            <figure>
                                <img src={process.env.PUBLIC_URL + '/' + "images/support-24-7.png"} alt="event-zone-img2" />
                            </figure>
                            <h3>24/7 support:</h3>
                            <p>We are available round the clock to support with any concern.</p>
                        </div>
                        <div className="index3-event-zone">
                            <figure>
                                <img src={process.env.PUBLIC_URL + '/' + "images/refunds.png"} alt="event-zone-img3" />
                            </figure>
                            <h3>Quick Refunds:</h3>
                            <p>All refunds will be processed within a fixed timeframe for hassle free refunds.</p>
                        </div>
                    </div>
                </div>
            </section>

            <section className="d-none blog-main-section index3-blog-section w-100 float-left padding-top position-relative">
                <div className="container">
                    <div className="generic-title2 text-center">
                        {/* <span className="small-text" data-aos="fade-up" data-aos-duration="700">SKILLS &amp; EXPERIENCE</span> */}
                        <span class="small-text">WHY MEETINGS21?</span>
                        <h2 className="mb-0" data-aos="fade-up" data-aos-duration="700">Our Services and Highlights</h2>
                    </div>
                    <div className="blogs-inner-con">
                        <div className="blog-box position-relative" data-aos="fade-up" data-aos-duration="700">
                            <div className="blog-img position-relative">
                                <span className="d-inline-block">01.</span>
                                <figure className="mb-0">
                                    <img src={process.env.PUBLIC_URL + '/' + "images/rear-view-of-audience-listening-speakers-on-the-stage-in-the-conference-hall-or-seminar-meeting.jpg"} alt="blog-img1" />
                                </figure>
                            </div>
                            <div className="blog-text">
                                {/* <span className="d-block">01.</span> */}
                                <span className="small-text aos-init aos-animate">21+ Hours of Content</span>
                                <h6 className="position-relative"><a href="single-blog.html">Listen to the 21 hours of quality presentations by the experts in the field.</a></h6>
                            </div>
                        </div>
                        <div className="blog-box position-relative" data-aos="fade-up" data-aos-duration="700">
                            <div className="blog-img position-relative">
                                <span className="d-inline-block">02.</span>
                                <figure className="mb-0">
                                    <img src={process.env.PUBLIC_URL + '/' + "images/young-happy-businesswoman-talking-on-microphone-while-attending-education-event-in-conference-hall-.jpg"} alt="blog-img3" />
                                </figure>
                            </div>
                            <div className="blog-text">
                                {/* <span className="d-block">Nov 18, 2020</span> */}
                                <span className="small-text aos-init aos-animate">World Class Speakers:</span>
                                <h6 className="position-relative"><a href="single-blog.html">Our conference features Top speakers as Plenary and Keynote Speakers.</a></h6>
                            </div>
                        </div>
                        <div className="blog-box position-relative" data-aos="fade-up" data-aos-duration="700">
                            <div className="blog-img position-relative">
                                <span className="d-inline-block">03.</span>
                                <figure className="mb-0">
                                    <img src={process.env.PUBLIC_URL + '/' + "images/female-office-employee-making-report-at-business-conference.jpg"} alt="blog-img3" />
                                </figure>
                            </div>
                            <div className="blog-text">
                                {/* <span className="d-block">Nov 25, 2020</span> */}
                                <span className="small-text aos-init aos-animate">Live Q&A:</span>
                                <h6 className="position-relative"><a href="single-blog.html">Get answers and Clarify your doubts directly from the experts.</a></h6>
                            </div>
                        </div>
                    </div>
                    <div className="mt-8"></div>
                    <div className="blogs-inner-con mt-5">
                        <div className="blog-box position-relative" data-aos="fade-up" data-aos-duration="700">
                            <div className="blog-img position-relative">
                                <span className="d-inline-block">04.</span>
                                <figure className="mb-0">
                                    <img src={process.env.PUBLIC_URL + '/' + "images/awards.jpg"} alt="blog-img1" />
                                </figure>
                            </div>
                            <div className="blog-text">
                                {/* <span className="d-block">01.</span> */}
                                <span className="small-text aos-init aos-animate">Conference & Awards:</span>
                                <h6 className="position-relative"><a href="single-blog.html">We recognize and award the top presentations in the conference.</a></h6>
                            </div>
                        </div>
                        <div className="blog-box position-relative" data-aos="fade-up" data-aos-duration="700">
                            <div className="blog-img position-relative">
                                <span className="d-inline-block">05.</span>
                                <figure className="mb-0">
                                    <img src={process.env.PUBLIC_URL + '/' + "images/24-7.jpg"} alt="blog-img3" />
                                </figure>
                            </div>
                            <div className="blog-text">
                                {/* <span className="d-block">Nov 18, 2020</span> */}
                                <span className="small-text aos-init aos-animate">24/7 support:</span>
                                <h6 className="position-relative"><a href="single-blog.html">We are available round the clock to support with any concern.</a></h6>
                            </div>
                        </div>
                        <div className="blog-box position-relative" data-aos="fade-up" data-aos-duration="700">
                            <div className="blog-img position-relative">
                                <span className="d-inline-block">06.</span>
                                <figure className="mb-0">
                                    <img src={process.env.PUBLIC_URL + '/' + "images/refunds.jpg"} alt="blog-img3" />
                                </figure>
                            </div>
                            <div className="blog-text">
                                {/* <span className="d-block">Nov 25, 2020</span> */}
                                <span className="small-text aos-init aos-animate">Quick Refunds:</span>
                                <h6 className="position-relative"><a href="single-blog.html">All refunds will be processed within a fixed timeframe for hassle free refunds.</a></h6>
                            </div>
                        </div>
                    </div>

                </div>
            </section>



            <section className="journey-section w-100 float-left padding-top grey-bg position-relative bg-white">

                <div className="container">
                    <div className="journey-inner-con">
                        <div className="journey-text-con">
                            <div className=" text-left ">
                                <h3 className="font-weight-bold mb-4">Why You Should Join Our Event? </h3>
                                <p className="">Attending our event not only gives you the opportunity to meet experts in the field but also gives exposure to new ideas that could be implemented in your own business.</p>
                            </div>
                            <div className="row">
                                <div className="col-6">
                                    <div className="custom-box-list">
                                        {/* <div className="mt-1 items-box text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-person-fill" viewBox="0 0 16 16">
                                                <path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6" />
                                            </svg>
                                        </div> */}
                                        <div>
                                            <h4><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-person-fill" viewBox="0 0 16 16">
                                                <path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6" />
                                            </svg> Networks with Peers:</h4>
                                            <p>Provides unparalleled networking opportunities where you can connect with professionals and researchers.</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-6">
                                    <div className="custom-box-list">
                                        {/* <div className="mt-1 items-box text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-people-fill" viewBox="0 0 16 16">
                                                <path d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6m-5.784 6A2.24 2.24 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.3 6.3 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 1zM4.5 8a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5" />
                                            </svg>
                                        </div> */}
                                        <div>
                                            <h4><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-people-fill" viewBox="0 0 16 16">
                                                <path d="M7 14s-1 0-1-1 1-4 5-4 5 3 5 4-1 1-1 1zm4-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6m-5.784 6A2.24 2.24 0 0 1 5 13c0-1.355.68-2.75 1.936-3.72A6.3 6.3 0 0 0 5 9c-4 0-5 3-5 4s1 1 1 1zM4.5 8a2.5 2.5 0 1 0 0-5 2.5 2.5 0 0 0 0 5" />
                                            </svg> Interactive Panel Discussions:</h4>
                                            <p>Participate in interactive panel discussions and workshops led by industry leaders during our conferences.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="row">
                                <div className="col-6">
                                    <div className="custom-box-list">
                                        {/* <div className="mt-1 items-box text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-briefcase-fill" viewBox="0 0 16 16">
                                                <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v1.384l7.614 2.03a1.5 1.5 0 0 0 .772 0L16 5.884V4.5A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5" />
                                                <path d="M0 12.5A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5V6.85L8.129 8.947a.5.5 0 0 1-.258 0L0 6.85z" />
                                            </svg>
                                        </div> */}
                                        <div>
                                            <h4><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-briefcase-fill" viewBox="0 0 16 16">
                                                <path d="M6.5 1A1.5 1.5 0 0 0 5 2.5V3H1.5A1.5 1.5 0 0 0 0 4.5v1.384l7.614 2.03a1.5 1.5 0 0 0 .772 0L16 5.884V4.5A1.5 1.5 0 0 0 14.5 3H11v-.5A1.5 1.5 0 0 0 9.5 1zm0 1h3a.5.5 0 0 1 .5.5V3H6v-.5a.5.5 0 0 1 .5-.5" />
                                                <path d="M0 12.5A1.5 1.5 0 0 0 1.5 14h13a1.5 1.5 0 0 0 1.5-1.5V6.85L8.129 8.947a.5.5 0 0 1-.258 0L0 6.85z" />
                                            </svg> Amazing Experience:</h4>
                                            <p>Discover the future of technology and science at Meetings21 conferences.</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-6">
                                    <div className="custom-box-list">
                                        {/* <div className="mt-1 items-box text-white">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-megaphone-fill" viewBox="0 0 16 16">
                                                <path d="M13 2.5a1.5 1.5 0 0 1 3 0v11a1.5 1.5 0 0 1-3 0zm-1 .724c-2.067.95-4.539 1.481-7 1.656v6.237a25 25 0 0 1 1.088.085c2.053.204 4.038.668 5.912 1.56zm-8 7.841V4.934c-.68.027-1.399.043-2.008.053A2.02 2.02 0 0 0 0 7v2c0 1.106.896 1.996 1.994 2.009l.496.008a64 64 0 0 1 1.51.048m1.39 1.081q.428.032.85.078l.253 1.69a1 1 0 0 1-.983 1.187h-.548a1 1 0 0 1-.916-.599l-1.314-2.48a66 66 0 0 1 1.692.064q.491.026.966.06" />
                                            </svg>
                                        </div> */}
                                        <div>
                                            <h4> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-megaphone-fill" viewBox="0 0 16 16">
                                                <path d="M13 2.5a1.5 1.5 0 0 1 3 0v11a1.5 1.5 0 0 1-3 0zm-1 .724c-2.067.95-4.539 1.481-7 1.656v6.237a25 25 0 0 1 1.088.085c2.053.204 4.038.668 5.912 1.56zm-8 7.841V4.934c-.68.027-1.399.043-2.008.053A2.02 2.02 0 0 0 0 7v2c0 1.106.896 1.996 1.994 2.009l.496.008a64 64 0 0 1 1.51.048m1.39 1.081q.428.032.85.078l.253 1.69a1 1 0 0 1-.983 1.187h-.548a1 1 0 0 1-.916-.599l-1.314-2.48a66 66 0 0 1 1.692.064q.491.026.966.06" />
                                            </svg> Great Speakers:</h4>
                                            <p>Hear presentations by future-shaping speakers who are leading experts in the field and gain insights from their work.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            {/* <div className="generic-btn" data-aos="fade-up" data-aos-duration="700">
                                <a href="about.html">READ MORE <i className="fas fa-arrow-right"></i></a>
                            </div> */}
                        </div>
                        <div className="journey-video-con d-inline-block">
                            <div className="video-inner-con">
                                <div className="learning-conference-video-box">
                                    <figure className="mb-0 d-inline-block">
                                        <img src={process.env.PUBLIC_URL + '/' + "images/why-img.png"} className="img-fluid" alt="index3-video-bg" />
                                    </figure>

                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </section>
        </div>

    );
};

export default Services;